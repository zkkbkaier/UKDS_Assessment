%%%%% Extension assessment method in which nondimensionalization is not applied (NNDEA_method)
clear;  
clc;
%%-------------------------------------------------------------------
...
%     bpp=max(bb')';          
    ...
    for i=1:n
        for j=1:m
            if x(i,jj)<=0.5*(a(i,j)+b(i,j))
               r(i,j)=2*(x(i,jj)-a(i,j))/(b(i,j)-a(i,j));
            else
               r(i,j)=2*(b(i,j)-x(i,jj))/(b(i,j)-a(i,j));
            end
        end
    end
    i=1:n;
    ...
    for i=1:n
        for j=1:m
            v=abs(b(i,j)-a(i,j));                                         
            s=abs(x(i,jj)-0.5*(a(i,j)+b(i,j)))-0.5*(b(i,j)-a(i,j));       
            sp=abs(x(i,jj)-0.5*(ap(i)+bp(i)))-0.5*(bp(i)-ap(i));          
            if a(i,j)==b(i,j)
               if x(i,jj)==a(i,j)
                  k(i,j)=0
               else 
                  k(i,j)=abs(x(i,jj)-a(i,j))/(sp-(abs(x(i,jj)-a(i,j)))); 
               end
            else
                if (x(i,jj)>=a(i,j) && x(i,jj)<=b(i,j))                     
                   k(i,j)=-s/v;                                            
                else
                   k(i,j)=s/(sp-s);                                        
                end
...
% string_1='nondimensionalized values of evaluation indices - xx';
% xlswrite('OutputData_NNDEA.xlsx',string_1,1,'A1');
% xlswrite('OutputData_NNDEA.xlsx',data111,1,'A2');

string_2='wo';
xlswrite('OutputData_NNDEA.xlsx',{string_2},2,'A1');
xlswrite('OutputData_NNDEA.xlsx',data222,2,'A2');

string_3='w';
xlswrite('OutputData_NNDEA.xlsx',{string_3},3,'A1');
xlswrite('OutputData_NNDEA.xlsx',data333,3,'A2');

string_4='k(i,j)';
xlswrite('OutputData_NNDEA.xlsx',{string_4},4,'A1');
xlswrite('OutputData_NNDEA.xlsx',data444,4,'A2');

string_5='kp';
xlswrite('OutputData_NNDEA.xlsx',{string_5},5,'A1');
xlswrite('OutputData_NNDEA.xlsx',data555,5,'A2');

string_6='j0';
xlswrite('OutputData_NNDEA.xlsx',{string_6},6,'A1');
xlswrite('OutputData_NNDEA.xlsx',data666,6,'A2');

string_7='jstar';
xlswrite('OutputData_NNDEA.xlsx',{string_7},7,'A1');
xlswrite('OutputData_NNDEA.xlsx',data777,7,'A2');
